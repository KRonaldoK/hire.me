...
public class UrlShortenerServiceContainerTest {
	
	
	  @SuppressWarnings("rawtypes")
	  @ClassRule public static GenericContainer container = new
	  GenericContainer(new ImageFromDockerfile() .withFileFromFile("Dockerfile",
	  new File(basePath(), "Dockerfile")) .withFileFromFile("target/us-srv.war",
	  new File(basePath(), "target/us-srv.war")))
	  .waitingFor(Wait.forHttp("/api/application.wadl")
	  .withStartupTimeout(Duration.ofSeconds(90))) .withLogConsumer(new
	  Slf4jLogConsumer(LoggerFactory.getLogger(UrlShortenerServiceContainerTest.
	  class))) .withExposedPorts(8080) .withExtraHost("localhost", "127.0.0.1");
	...
	public void setUp() {
		...
		String uriRedirect = String.format("http://%s:%s/api",
											container.getContainerIpAddress(), container.getMappedPort(80));
		apiRedirect = client.target(uriRedirect);
		
		String uriShorten = String.format("http://%s:%s/api/urlRepository",
											container.getContainerIpAddress(), container.getMappedPort(80));
	...
	public void shortenUrlNoCustomAlias() {
		...
		//
		String longUrl = "http://www.raialeve.com.br";
	...
	private static String basePath() {
		URL resource = UrlShortenerServiceContainerTest.class.getResource("/");
		File file;
		try {
